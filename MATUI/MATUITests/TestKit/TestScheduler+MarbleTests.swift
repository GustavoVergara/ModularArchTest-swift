...
extension TestScheduler {
    ...
    */
    ...
    /**
     ...
     - returns: Observable sequence specified by timeline and values.
    */
    func createObservable<T>(_ events: [[Recorded<Event<T>>]], resolution: TimeInterval = 1) -> Observable<T> {
        var attemptCount = 0
    ...
    */
    func record<O: ObservableConvertibleType>(source: O, disposeAt timeToDispose: TestTime = 100000) -> TestableObserver<O.E> {
        let observer = self.createObserver(O.E.self)
        let disposable = source.asObservable().bind(to: observer)
        self.scheduleAt(timeToDispose) {
            disposable.dispose()
        }
        return observer
    }
    ...
    /**
     ...
     - parameter sequence: Observable sequence on witch events will be runned.
     ...
     */
    public func start<O: ObservableConvertibleType>(created: TestTime = Defaults.created, subscribed: TestTime = Defaults.subscribed, disposed: TestTime = Defaults.disposed, sequence: O) -> TestableObserver<O.E> {
        return self.start(created: created, subscribed: subscribed, disposed: disposed, create: { sequence.asObservable() })
    }

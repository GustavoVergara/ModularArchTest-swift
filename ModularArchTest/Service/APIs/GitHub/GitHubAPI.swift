...
struct GitHubAPI {
    ...
    func getRepositories(ownerUsername: String?, type: RepoType = .all, sort: RepoSortType = .updated, order: SortOrder = .desc) -> Single<[Repository]> {
    ...
    func searchIssues(repositoryFullName: String, state: Issue.State? = nil, sort: SearchIssuesSortType = .updated, order: SortOrder = .desc) -> Single<SearchResult<Issue>> {
